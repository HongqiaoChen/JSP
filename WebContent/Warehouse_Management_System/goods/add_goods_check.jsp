	...
	<%
		...
		ResultSet rs=null;
			try {
				...
				String sql2 = "insert into goods(id,length,width,heigth) values('"+id+"','"+length+"','"+width+"','"+heigth+"')";
				rs = stmt.executeQuery(sql1);
				if (rs.next()) {
					response.sendRedirect("add_goods_error.jsp");
					
				}else {
					int count = stmt.executeUpdate(sql2);
					//处理结果集
					if(count>0) {
						response.sendRedirect("add_goods_success.jsp");
					}
				}
